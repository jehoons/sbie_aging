...
"""
사용하고 싶은 ablist가 따로 있다면 아래의 코드 사용 바람
ablist = []
...
genelist = list(data_gene.values)
...
#simulation with no interpolation
#intdatas = conddatas
...
for gene in genelist:
    if 'nan' in str(gene):
        continue
#    print(gene)
    single_gene_list.extend(gene.split(' '))
    
gene_com =  ac.c(list(set(single_gene_list)))
#print(gene_com)
ab_com = []
for com in gene_com:
#    print(com)
    ab_com.append(am.mapping(com,ablist,genelist))


#validating seneR using rapa data
file_name = 'ab_senR_result '
h = open('%s.txt'%file_name,'w')
s_err_total = []
q_err_total = []
for u,KD in enumerate(ab_com):
    print(float(u/len(ab_com)*100))
    ...
    for item in KD:
        h.write("%s\t" % item)
    h.write('\n')
    h.write('knock down gene : ')
    for item in gene_com[u]:
        h.write("%s\t" % item)
    h.write('\n')
