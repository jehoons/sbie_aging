...
if mode:
    ...
    random_totdatano = np.random.choice(totdatano,totdatano)
    for kk, datano in enumerate(random_totdatano):
        ### randomize the w with same sign amd b
#        ws[datano] = random.uniform(0.1,10)*np.array(paramsdata[datano * 3 + 4], dtype=np.float64)\
#        /np.array([abs(float(x)) for x in paramsdata[datano * 3 + 4]], dtype=np.float64)
#        bs[datano] = random.uniform(0.1,10)*np.array(paramsdata[datano * 3 + 5], dtype=np.float64)\
#        /np.array([float(x) for x in paramsdata[datano * 3 + 5]], dtype=np.float64)
        ### randomize the w with same sign amd b
#        ws[datano] = random.uniform(0.1,10)*np.array(paramsdata[datano * 3 + 4], dtype=np.float64)\
#        /np.array([abs(float(x)) for x in paramsdata[datano * 3 + 4]], dtype=np.float64)
#        bs[datano] = random.uniform(0.1,10)*np.array(paramsdata[datano * 3 + 5], dtype=np.float64)\
#        /np.array([abs(float(x)) for x in paramsdata[datano * 3 + 5]], dtype=np.float64)
#        ### normal control target
        ws[kk] = np.array(paramsdata[datano * 3 + 4], dtype=np.float64)
        bs[kk] = np.array(paramsdata[datano * 3 + 5], dtype=np.float64)
#   
#    
...
if mode:
    ...
    def plotdata(tgtdict,nn):
        ...
        #plt.tight_layout()
        plt.show()
        plt.savefig('%s_reversion.jpg'%nn)
