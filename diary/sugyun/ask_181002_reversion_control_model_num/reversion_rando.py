...
#        rn = np.random.choice(totdatano,n,replace = False)
#        start_index = [4+3*x for x in rn]
#        end_index = [7+3*x for x in rn]
#        data_2 = data[0:4]
#        for s,e in zip(start_index,end_index):
#            data_2.extend(data[s:e])
#        return (data_2)
#    def random_choice_exclusive(data,n,ss):
#        totdatano = int((len(data) - 4) / 3)
...
#    
    ...
    if mode:
        ...
        plotdata(tuplemode)
        import pandas as pd
        r = pd.DataFrame(tuplemode, index = ['count'])
        r = pd.DataFrame.transpose(r)
    #    r = r.sort_values(by=['count'], ascending=[False])
        pd.DataFrame.to_csv(r,str(ii)+'sample'+str(n)+'.csv')
