...
class OptWeight(base):
    ...
    def __init__(self, dim = 9,i_dim = 9):  # num of link 13(weight) + 11(basal)
        ...
        for i in initial_bin:
            ...
            self.initial_state.append([int(s) for s in default])
        ...
        quo, re = divmod(self.n_node, cpu_num)
        ...
        for i in range(self.sample_size):
            ...
            self.initial_list.append(initial_state)

    def get_attractor(self, Weight, initial_state, state_traj, pert_node=[], state = []):
    ...
    ### Model
    def _func(self, w, initial_attractor, pert=[],state=[]):
        # make weight matrix
    ...
    def _objfun_impl(self, params):
        ...
        attractor_set = []
        attractor_dic = {}
        for initial in self.initial_list:
            result_att = self._func(w, initial, pert=list(self.input_dic.keys()),state=list(self.input_dic.values()))
        ...
        for initial_att in attractor_set:
            for i in range(self.n_node):
                final_att = self._func(w, initial_att, pert=list(self.input_dic.keys())+[int(i)], state = list(self.input_dic.values())+[0])
                ...
                final_att = self._func(w, initial_att, pert=list(self.input_dic.keys())+[int(i)], state=list(self.input_dic.values())+[1])
                if type(final_att[0]) is int:
        ...
        for state in self.train_data:
            ...
            total_s += int(min(s))
        total_s = total_s + self.lam*sum([abs(x) for x in w])
        return (total_s,)
