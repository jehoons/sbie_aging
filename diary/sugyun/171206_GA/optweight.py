...
class OptWeight(base):
    ...
    def __init__(self, dim = 13, i_dim=13):  # num of link
        ...
        self.basal = [0, 0, 0, -1, -2, 0, 0, 0, 0, 0, 0]
        self.node_list = []
        self.edge_list = []
        ...
        lb[:] = 1
        ub[:] = 3  # ub= 1 + max(degree) & larger than basal
    ...
    def _func(self, w, k):
        ...
        for (enum, e) in enumerate(self.edge_list):
            ...
            a[row][col] = w[y]
            y += 1
        a = np.vstack((a, np.array(self.basal)))
    ...
    def _objfun_impl(self, params):
        ...
        for state in self.data:
            ...
            y = self._func(w, k)
            # ex) y = [1, 0, 1, 1, 1]
            s += sum((obj_state[i] - y[i])**2 for i in range(len(y)))
