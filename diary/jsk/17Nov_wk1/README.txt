...
4. MI_results에는 MI_calc에 대한 결과들이 들어 있고 network_pruning_result들에는 각각의 cutoff에 대한 pruning_analysis의 결과들이 들어있다.
5. agingnetworkp들은 network_pruning_results들에 있는 모든 네트워크를 반영하는 최종 네트워크이다. 이를 만드는 코드가 net_network_gen이다