...
dataname = 'obj_attractor_control_ps.csv'
...
tempconlist = conlist.copy()
for idx in range(len(conlist)):
    tempconlist[idx] = tempconlist[idx] + '_'
conlist = np.concatenate((conlist, tempconlist))
...
for ws, bs in netparamslist:
    ...
    for drugtgt in range(1, maxdrugtgt + 1):
        ...
        for contgt in contgts:
            ...
            for tgt in contgt:
                if tgt[-1] == '_':
                    conbs[node_list.index(tgt[:-1])] = -99
                else:
                    conbs[node_list.index(tgt)] = 99
...
if mode:
    ...
    def plotdata(tgtdict):
        ...
        ax.set_title('Proliferation to Senescence Control: Both (211 Models)', fontsize=20)
        plt.tight_layout()
        plt.show()
