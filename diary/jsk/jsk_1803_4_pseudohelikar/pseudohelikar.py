...
dosagebinsize = .1#bin size of drug dosage
...
datafilename = 'obj_attractor_ss.csv'
...
if len(inhibparams) <= 1:
    ...
    fig = plt.figure()
    width = .05
    plt.bar(np.arange(len(drugeffarr)) * dosagebinsize, drugeffarr, width)
    ...
    if mode <= 0:
        ...
        plt.ylabel('Mode of fitness less than ' + str(thres))
    if datafilename[-5] == 'q':
