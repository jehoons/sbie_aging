...
class OptWeightandBasal(base):
    ...
    def __init__(self, dim = 1, i_dim = 1, netdata = netdata, conddata = conddata):
        ...
        lb[:] = 1
        ub[:] = self.maxindeg#weight resolution; set as maxindegree(account for all AND at maxindegree, with negative basal)
    ...
    def _func(self, w, b, inistates):
        ...
        while 1:#state transition from t to t + 1
            ...
            if transed_state in state_history:#get the point attractor and cyclic attractor; list of state trajectory since the redundant state
                return state_history[state_history.index(transed_state):]#point attractor has list length of 1
                
            else:#save the transient state and move on
    ...
    def _objfun_impl(self, params):#params is edge weight and node basal level in that order
        w = params[0:self.num_edges]#weight parameters
        b = list(np.array(params[self.num_edges:self.num_edges+self.num_nodes])-self.maxindeg)#basal activity parameter; boundary repositioning for basal activites
        fitness = 0.0
