...
for poisson_lambda in np.arange(poisson_lambda_min, poisson_lambda_max+poisson_lambda_step, poisson_lambda_step):
    ...
    if plot_rank_movement:
        ...
        if plot_save:
            plt.savefig('node_rank_movement.jpg')
...
if plot_3d:
    ...
    if plot_save:
        plt.savefig('3d_projection.jpg')

if plot_lineplot:
    plt.figure()
    if plot_lineplot_shadow:
        ...
        g = sns.catplot(x='nodes_added', y='kendall_tau', hue='lambda', kind='point', palette=sns.color_palette('deep', len(lambdas_2_plot)), ci='sd', capsize=.15, aspect=1.5, data=pd.concat(dfs, ignore_index=True), legend='full')
        g.despine(left=True)
    if plot_save:
        g.savefig('lineplot.jpg')
