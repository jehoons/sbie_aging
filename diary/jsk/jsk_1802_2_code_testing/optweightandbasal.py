...
rec.write('optweightandbasal class import start')#
rec.write('\n')#
...
forcew = [['ULK1', 'ATG13', 2],
          ...
          ['ROS', 'DNAdamage', 0],
          ['TP53', 'IGF1', 0],
          ['INSR', 'IRS1', 5],#basal activity set to 5to give half chance of influencing X; basal activity bound (1~9)
          ['IGF1R', 'IRS1', 2],
          ['S6K1', 'IRS1', 1],
          ['KRAS', 'PIK3CA', 1],
          ['IRS1', 'PIK3CA', 2]
...
rec.write('forcew and forceb declared:')#
rec.write(str(forcew))#
rec.write('\n')#
rec.write(str(forceb))#
rec.write('\n')#
...
class OptWeightandBasal(base):
    ...
    def __init__(self, dim = dim, i_dim = dim, netdata = netdata, conddata = conddata, iptidx = iptidx, forcew = forcew, forceb = forceb, netfilename = netfilename):
        ...
        self.forcew = forcew
        self.forceb = forceb
        rec.write('class declared;__init__')#
        rec.write('\n')#
        ...
        rec.write('maximum indegree:')#
        rec.write(str(self.maxindeg))#
        rec.write('\n')#
        ...
        rec.write('parameter bounds; unadjusted:')#
        rec.write(str(self.wr))#
        rec.write('\n')#
        rec.write('end of __init__')#
        rec.write('\n')

    ###the Boolean network model
    def _func(self, w, b, inistates):
        rec.write('_func called')#
        rec.write('\n')#
    ...
    def _objfun_impl(self, params):#params is edge weight and node basal level in that order
        ...
        for condno in range(self.num_conds):#for all of the conditions presented calculate the cost function and add all
            ...
            rec.write('predicted attractors:')#
            rec.write(str(predstateslist))#
            rec.write('\n')#
            ...
            rec.write('objective phenotype:')#
            rec.write(str(objphen))#
            rec.write('\n')#
            ...
            for predstates in predstateslist:
                rec.write('attractor list for loop start')#
                rec.write('\n')#
                predstates = np.array(predstates)
                rec.write('predicted states:')#
                rec.write(str(predstates))#
                rec.write('\n')#
                nodelist = np.array(self.node_list)
                phennode = list(nodelist[objboolmask])
                rec.write('objective phenotype nodes')#
                rec.write(str(phennode))#
                rec.write('\n')#
                predphen = list(predstates[objboolmask])
                rec.write('predicted phenotype:')#
                rec.write(str(predphen))#
                rec.write('\n')#
                ...
                rec.write('partial fitness for this condition:')#
                rec.write(str(tmpfit))#
                rec.write('\n')#
            rec.write('attractor list for loop end')#
            rec.write('\n')#
            fitness += np.mean(np.array(tmpfitlist))#fitness is the mean of fitness of all the states in a cyclic attractor
        rec.write('final fitness:')#
        rec.write(str(fitness))#
        rec.write('\n')#
        return (fitness,)
