...
for pdk1_inhibit, positive_feedback_network_only in zip([False, True, False, True], [False, False, True, True]):
    for pos_feed_cut1, pos_feed_cut2 in zip([False, True, False, True], [False, False, True, True]):
        ...
        for pdk1_force in [(False, False), (False, True), (True, False), (True, True)]:
            ...
            for _ipt_cond in ["{0:b}".format(i).zfill(3) for i in range(2**3)]:
                ...
                for (ws, bs), attractor_prev in zip(netparamslist, attractorlist):
                    ...
                    #cut positive feedback
                    if pos_feed_cut1:
                        ws[pos_feed_link_idx1] = 0
                    if pos_feed_cut2:
                        ws[pos_feed_link_idx2] = 0
            ...
            if pos_feed_cut1:
                pos_feed_cut_str1 = 'cut'
            else:
                pos_feed_cut_str1 = 'intact'
            if pos_feed_cut2:
                pos_feed_cut_str2 = 'cut'
            else:
                pos_feed_cut_str2 = 'intact'
            if pdk1_controlled_network_only:
            ...
            all_df.to_csv('results/simulation_result_pdk1_{}_bf_{}_af_{}_akt1_pten_{}_ikbkb_pten_{}{}.csv'.format(pdk1_force_str, int(pdk1_force[0]), int(pdk1_force[1]), pos_feed_cut_str1, pos_feed_cut_str2, netparams_str))
            attractor_avg_new_df.to_csv('results/average_attractor_pdk1_{}_bf_{}_af_{}_akt1_pten_{}_ikbkb_pten_{}{}.csv'.format(pdk1_force_str, int(pdk1_force[0]), int(pdk1_force[1]), pos_feed_cut_str1, pos_feed_cut_str2, netparams_str))
