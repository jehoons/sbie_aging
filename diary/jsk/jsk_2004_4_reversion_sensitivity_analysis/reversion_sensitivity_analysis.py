...
for mode in ['edge', 'node']:
    for dataname in ['obj_attractor_control_sq.csv', 'obj_attractor_control_sp.csv']:
        ...
        disruption_step = 1
        repeat = 30
        for disruption in range(0, max_disruption + 1, disruption_step):
            for s in range(repeat):
                if disruption == 0 and s > 0:
                    continue
                print('{}_{} sensitivity: {}/{}, iteration: {}/{}'.format(mode, dataname[-6:-4], disruption, max_disruption, s+1, repeat))
                np.random.seed(s)
                ...
                def generate_fake_link(netdata_):
                    tmp_link = netdata_[0].copy()
                    while tmp_link in netdata_:
                        _iptnode, _optnode = np.random.choice(signalnodes, 2, replace=False)
                        _interaction = np.random.choice(['activate', 'inhibit'])
                        tmp_link = [_iptnode, _interaction, _optnode]
                    return tmp_link
                
                def generate_fake_node(netdata_, node_name, candidate_nodes):
                    links = []
                    poisson_lambda = 2.125#1.878 or 2.125
                    #choosing indegrees
                    num_indeg = 0
                    while not(num_indeg):
                ...
                for netnum, (ws, bs) in enumerate(netparamslist):
                    ...
                    if mode == 'edge':
                        num_added_links = disruption
                        for i in range(disruption):
                            #add links and adjust weights
                            _link = generate_fake_link(_netdata)
                            _netdata.append(_link)
                            ws = np.append(ws, np.random.rand() * 18)
                    ...
                    if mode == 'node':
                        ...
                        for i in range(disruption):
                            ...
                            _links = generate_fake_node(_netdata, node_name_, signalnodes_)
                            #add links and adjust weights
                ...
                sensitivity_result = pd.concat([sensitivity_result, pd.Series(tuplemode, name='{}_added_{}_{}'.format(mode, disruption, s))], axis=1, sort=True).fillna(0)
        
        sensitivity_result.to_csv('sensitivity_result_{}_{}.csv'.format(mode, dataname[-6:-4]))
