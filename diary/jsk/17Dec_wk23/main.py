...
num_gens = 2000
prob = OptWeight(dim=128, i_dim=128)#dim=nodeno + edgeno
algo = algorithm.jde(gen=num_gens)  # de, sga
pop = population(prob, 500)  # the last argument is population size.
isl = island(algo, pop) # island # != pop #
algo_name = algo.get_name()

isl.evolve(1) # evolve island 10 times
isl.join()
