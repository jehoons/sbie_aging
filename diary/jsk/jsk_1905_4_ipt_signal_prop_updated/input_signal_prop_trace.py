...
attractor_avgs = []
state_names = []
ss_comprehensive = np.array(netdata)
for iptcond in ['s', 'q', 'p']:
    ...
    avgws = np.mean(np.array(netparamslist)[:,0])
    signal_strength = []
    for link, w in zip(netdata, avgws):
        if link[1] == 'inhibit':
            w = -w
        state = 2 * attractor_avg[node_list.index(link[0])] - 1
        signal_strength.append(state * w)
    ss_comprehensive = np.hstack((np.array(ss_comprehensive),np.array(signal_strength).reshape(len(netdata),1)))

pd.DataFrame(np.array(attractor_avgs).T, index=node_list, columns=state_names).to_csv('attractor_avgs.csv')
pd.DataFrame(ss_comprehensive, columns=['input', 'interaction', 'output']+state_names).to_csv('signal_strength.csv')
