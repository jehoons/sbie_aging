...
totnetno = 8#number of networks; this should be consistent with actual number of PKN_20 variants
netno = random.randrange(1, totnetno + 1)
...
for idx, state in enumerate(list(conddata.iloc[0])):
    if state != 9:
        iptidx.append(idx)
forcew = []#index and value of weight I want to manually adjust
forceb = []#[[list(conddata.columns).index('n6'), 6]]#index and value of weight I want to manually adjust
...
class OptWeightandBasal(base):
    ...
    def _objfun_impl(self, params):#params is edge weight and node basal level in that order
        ...
        b = list(np.multiply(b, bmask))#adjusting the boundaries to cover negative basal activity levels
        ...
        for condno in range(self.num_conds):#for all of the conditions presented calculate the cost function and add all
            ...
            for fw in self.forcew:#fw[0] is the index and fw[1] is the value to replace
                w[fw[0]] = fw[1]
            for fb in self.forceb:#fb[0] is the index and fb[1] is the value to replace
                b[fb[0]] = fb[1]
