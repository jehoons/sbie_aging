...
def nparr2str(nparr):
    rtstr = ''
    for elem in nparr:
        rtstr=rtstr+str(elem)
    return rtstr

def str2nparr(string):
    rtnparr = np.zeros(len(string))
    for idx, elem in enumerate(string):
        rtnparr[idx] = float(elem)
    return rtnparr
...
mis = pd.read_csv('nodemi.csv', index_col=0)
misbool = mis[mis > .05]
sene = misbool['Senescence'].dropna()
qui = misbool['Quiescence'].dropna()
pro = misbool['Proliferation'].dropna()
misboolnot = mis[mis >.00001][mis < .05]
senenot = list(misboolnot['Senescence'].dropna().index)
quinot = list(misboolnot['Quiescence'].dropna().index)
pronot = list(misboolnot['Proliferation'].dropna().index)
relnds = np.array(list(set(sene.index) or set(qui.index) or set(pro.index)))
...
for k in range(3):
    ...
    if k==0:
        ...
        print(pronot)
    #plt.title(label)
    idxmask = np.array([True if nd in nodes4mask else False for nd in node_list])
    
    attlist = []
    for attractors in attractorlist:
        atts = np.zeros((3, len(nodes4mask)))
        atts[k] = np.mean(attractors[k], axis=0)[idxmask]
        att = atts[k].copy()
        attlist.append(att)
    ...
    attlist = []
    for attractors in attractorlist:
        att = np.zeros((3, len(relnds)))
        for i, attractor in enumerate(attractors):
            att[i] = np.mean(attractor, axis=0)[idxmask]
        att = att.flatten()
        attlist.append(att)
    attlist = np.array(attlist)
    '''
    '''
    nomod = 1000#len(attlist)
    models = np.arange(1, nomod + 1)
    unimodels = [len(np.unique(attlist[:i+1], axis=0)) for i in range(nomod)]
    plt.plot(models, unimodels, models, models)
    '''
    numvis=15
    ypos=np.arange(numvis)
    uniatts, nouniatt = np.unique(attlist, axis=0, return_counts=True)
    uniattlist = []
    for i, j in zip(uniatts, nouniatt):
        uniattlist.append([j, i])
    #nouniatt = sorted(nouniatt, reverse=True)
    #plt.barh(ypos, nouniatt[:numvis])
    uniattlist.sort(key=lambda x: x[0], reverse=True)
    
    tgtmod = uniattlist[0][1].copy()
    modnum=5
    modidx=np.zeros(modnum, dtype=np.int32)
    idx=0
    idx2=0
    while modnum > 0:
        if (attlist[idx] == tgtmod).all():
            modidx[idx2]=idx
            modnum -= 1
            idx2 += 1
        idx += 1
    
    models.append(np.array(netparamslist)[modidx].copy())
    modws=[]
    modbs=[]
    for model in models[k]:
        modws.append(model[0])
        modbs.append(model[1])
    pd.DataFrame(modws).to_csv('model_ws_' + label + '.csv')
    pd.DataFrame(modbs, columns=node_list).to_csv('model_bs_' + label + '.csv')
